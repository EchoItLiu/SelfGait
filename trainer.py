...
class BYOLTrainer:
    ...
    def update(self, batch_view_1):
        ...
        predictions_from_view_1= self.predictor(self.projection_1(self.TP_1(self.conv_trans(self.online_network(*seq_1,batch_frame_1)))))
        predictions_from_view_2= self.predictor(self.projection_1(self.TP_1(self.conv_trans(self.online_network(*seq_2,batch_frame_2)))))
        
        # compute key features
        with torch.no_grad():
            targets_to_view_2= self.projection_2(self.TP_2(self.target_network(*seq_1,batch_frame_1)))
            targets_to_view_1= self.projection_2(self.TP_2(self.target_network(*seq_2,batch_frame_2)))
        loss = self.regression_loss(predictions_from_view_1, targets_to_view_1)
        loss += self.regression_loss(predictions_from_view_2, targets_to_view_2)
        return loss.mean()
        
    
    def train_ft(self, train_dataset,maxiter=10000):
        train_source=train_dataset
        #train_loader = DataLoader(train_dataset, batch_size=self.batch_size,num_workers=self.num_workers, drop_last=False, shuffle=True)
        for subnet in self.Encode_list:
            subnet.train()
        ...
        self.initializes_target_network()
        train_label_set = list(train_dataset.label_set)
        ...
        for epoch_counter in range(self.max_epochs):
            ...
            for batch_view in train_loader:
                ...
                loss = self.update_ft(batch_view,train_label_set)
                self.writer.add_scalar('loss', loss, global_step=self.niter)
                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()
    ...
    def update_ft(self, batch_view_1,train_label_set):
        ...
        if(self.niter%5==0):
            ...
            loss_ft=loss
            self.writer.add_scalar('loss_ft', loss_ft, global_step=self.niter)
    ...
    def test(self,test_dataset,niter=-1):
        ...
        print('Transforming...')
        time = datetime.now()
        test = self.transform(test_dataset, batch_size=1)
        print('Evaluating...')
